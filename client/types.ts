...
// Bản ghi audit log thật từ backend (GET /api/audit-logs) — khác SystemLog (kiểu cũ tham khảo từ persistence.ts):
// action chỉ còn đúng 7 giá trị theo CLAUDE.md mục 4, chi tiết nghiệp vụ nằm trong entityType/details.
export interface AuditLogEntry {
  id: string;
  timestamp: string;
  userId: string | null;
  userName: string;
  action: 'INSERT' | 'UPDATE' | 'DELETE' | 'APPROVE' | 'REJECT' | 'LOGIN' | 'LOGOUT';
  entityType: string;
  details: string;
  success: boolean;
}

export interface AllowedTabsResponse {
  user: User;
  allowedTabs: string[];
}
