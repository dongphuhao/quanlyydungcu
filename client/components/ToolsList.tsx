...
import { StatusBadge, TOOL_STATUS_BADGE } from '../constants';
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {filteredTools.length === 0 ? (
              <tr><td colSpan={6}><EmptyState title="Không tìm thấy dụng cụ" description="Thử đổi từ khoá tìm kiếm hoặc bộ lọc loại dụng cụ." /></td></tr>
            ) : filteredTools.map(tool => (
              <tr key={tool.id} className="hover:bg-gray-50/70 transition-colors">
                ...
                <td className="px-6 py-4">
                  ...
                  <span className={`font-semibold ${tool.availableQuantity < 5 ? 'text-red-500' : 'text-gray-700'}`}>{tool.availableQuantity}</span>
                </td>
                <td className="px-6 py-4">
                  <StatusBadge map={TOOL_STATUS_BADGE} status={tool.status} />
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center space-x-2">
                    <button onClick={() => onEditTool(tool)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Sửa thông tin">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                    <button onClick={() => onAdjustStock(tool, 'restock')} className="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-bold rounded hover:bg-blue-100 transition-colors">
                      + Nhập
                    </button>
