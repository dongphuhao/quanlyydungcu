...
import { MedicalTool, BorrowForm, ToolKit, BorrowItem, User, AuditLogEntry, BorrowStatus, Department, ToolCategory, SterilizationLog, LiquidationLog } from './types';
import * as api from './services/api';
import { getInventoryInsights } from './services/geminiService';
import { StatusBadge, BORROW_STATUS_BADGE } from './constants';
import EmptyState from './components/EmptyState';
import * as XLSX from 'xlsx';

// safeFetch: một API 403 (vd. GET /users với role không phải admin) không được làm sập cả refreshData —
// coi như "không có quyền xem" tương đương danh sách rỗng, đúng tinh thần allowedTabs đã ẩn tab đó ở UI rồi.
function safeFetch<T>(p: Promise<T[]>): Promise<T[]> {
  return p.catch(() => []);
}
...
const App: React.FC = () => {
  ...
  const [tools, setTools] = useState<MedicalTool[]>([]);
  const [borrows, setBorrows] = useState<BorrowForm[]>([]);
  ...
  const [liquidationLogs, setLiquidationLogs] = useState<LiquidationLog[]>([]);
  ...
  // Khôi phục phiên đăng nhập (session cookie) khi tải lại trang.
  useEffect(() => {
    api.me().then((res) => {
      if (res) {
        setCurrentUser(res.user);
        setAllowedTabs(res.allowedTabs);
        setActiveTab(res.allowedTabs[0] ?? 'dashboard');
      }
      setAuthChecked(true);
    });
  }, []);
  ...
  const refreshData = async () => {
    const [toolsData, borrowsData, kitsData, logsData, deptData, catData, usersData, stData, liqData] = await Promise.all([
      safeFetch(api.getTools()),
      safeFetch(api.getBorrows()),
      safeFetch(api.getKits()),
      safeFetch(api.getAuditLogs()),
      safeFetch(api.getDepartments()),
      safeFetch(api.getCategories()),
      safeFetch(api.getUsers()),
      safeFetch(api.getSterilizationLogs()),
      safeFetch(api.getLiquidationLogs()),
    ]);
    setTools(toolsData);
    ...
    setUsers(usersData);
    setSterilizationLogs(stData);
    setLiquidationLogs(liqData);
  };
  ...
  const handleLogin = (res: { user: User; allowedTabs: string[] }) => {
    setCurrentUser(res.user);
    setAllowedTabs(res.allowedTabs);
    setActiveTab(res.allowedTabs[0] ?? 'dashboard');
  };
  ...
  const handleRequestBorrow = async (borrower: string, dept: string, items: BorrowItem[]) => {
    try {
      await api.requestBorrow(borrower, dept, items);
  ...
  const handleStartSterilization = async (logId: string) => {
    try {
      await api.startSterilization(logId);
      await refreshData();
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleCompleteSterilization = async (logId: string) => {
    try {
      await api.completeSterilization(logId);
      await refreshData();
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleLiquidate = async (quantity: number, reason: string, notes: string) => {
    if (!liquidatingTool) return;
    try {
      await api.liquidateTool(liquidatingTool.id, quantity, reason, notes);
  ...
  const getStatusBadge = (status: BorrowStatus) => <StatusBadge map={BORROW_STATUS_BADGE} status={status} />;
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-4">
            <div className="flex justify-end space-x-2">
              <button onClick={() => exportToExcel(tools, 'Danh_sach_dung_cu')} className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-sm font-semibold border border-blue-100 hover:bg-blue-100 transition-colors">
                Xuất Excel
              </button>
            </div>
              <ToolsList 
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex space-x-2">
                <button onClick={() => exportToExcel(kits, 'Danh_sach_goi_dung_cu')} className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-sm font-semibold border border-blue-100 hover:bg-blue-100 transition-colors">
                  Xuất Excel
                </button>
            ...
            {kits.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
                <EmptyState title="Chưa có gói dụng cụ nào" description="Nhấn “+ Thiết lập gói mới” để tạo gói dụng cụ đầu tiên." />
              </div>
            ) : (
                ...
                <div key={kit.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col hover:shadow-xl transition-all group">
                  <div className="p-6 border-b border-gray-50">
                    ...
                    <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden flex mb-2">
                      <div style={{ width: `${(kit.inStockQuantity / kit.totalQuantity) * 100}%` }} className="bg-blue-600 h-full" title="Trong kho"></div>
                      <div style={{ width: `${(kit.borrowedQuantity / kit.totalQuantity) * 100}%` }} className="bg-blue-300 h-full" title="Đang mượn"></div>
                      <div style={{ width: `${(kit.waitingSterilizationQuantity / kit.totalQuantity) * 100}%` }} className="bg-gray-400 h-full" title="Chờ tiệt trùng"></div>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px]">
                      ...
                      <div className="flex items-center justify-between"><span className="text-blue-700">Trong kho:</span> <span className="font-bold text-blue-700">{kit.inStockQuantity}</span></div>
                      <div className="flex items-center justify-between"><span className="text-blue-500">Đang mượn:</span> <span className="font-bold text-blue-500">{kit.borrowedQuantity}</span></div>
                      <div className="flex items-center justify-between"><span className="text-gray-500">Tiệt trùng:</span> <span className="font-bold text-gray-500">{kit.waitingSterilizationQuantity}</span></div>
                    </div>
                  </div>
      ...
      case 'borrow_request':
        return (
          <div className="space-y-6">
            ...
            {kits.filter(k => k.inStockQuantity > 0).length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
                <EmptyState title="Hiện không có gói nào sẵn sàng" description="Tất cả gói dụng cụ đang được mượn hoặc xử lý. Vui lòng thử lại sau." />
              </div>
            ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {kits.filter(k => k.inStockQuantity > 0).map(kit => (
                <div key={kit.id} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all group flex flex-col">
                  ...
                  <div className="text-xs font-bold text-blue-700 mb-4 bg-blue-50 px-3 py-2 rounded-lg inline-flex items-center space-x-2">
             ...
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                 ...
                 <button
                    ...
                    className="px-3 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700 active:scale-[0.97] transition-all flex items-center space-x-1"
                  >
               ...
               <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {myBorrows.length === 0 ? (
                      <tr><td colSpan={5}><EmptyState title="Không tìm thấy yêu cầu mượn nào" description="Thử đổi bộ lọc trạng thái, khoa phòng hoặc khoảng thời gian." /></td></tr>
                    ) : (
                      myBorrows.map(b => (
                          ...
                          <td className="px-6 py-4">
                            <div className="text-[10px] space-y-2">
                              ...
                              {b.items.filter(it => it.type === 'tool').length > 0 && (
                                <div className="mt-2">
                                  <div className="text-gray-500 font-black uppercase mb-1">Dụng cụ lẻ:</div>
                                  <div className="space-y-1">
                                    {b.items.filter(it => it.type === 'tool').map((it, i) => (
                                      <div key={i} className="flex justify-between items-center text-gray-700 font-medium p-1 bg-gray-50 rounded">
                                        <span>• {it.name}</span>
                                        <span className="font-bold text-gray-600">x{it.quantity}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              {b.status === 'Requested' && (currentUser?.role === 'admin' || currentUser?.role === 'manager') && (
                                <button
                                  ...
                                  className="px-3 py-1 bg-blue-600 text-white text-[10px] font-bold rounded hover:bg-blue-700 transition-all"
                                >
                                  Duyệt mượn
                                </button>
                              )}
                              ...
                              <button
                                ...
                                className="px-3 py-1 bg-gray-50 text-gray-600 text-[10px] font-bold rounded hover:bg-gray-600 hover:text-white transition-all border border-gray-200 flex items-center space-x-1"
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {pendingApprovals.length === 0 ? (
                      <tr><td colSpan={5}><EmptyState title="Không có yêu cầu nào cần phê duyệt" description="Yêu cầu mượn/trả mới sẽ xuất hiện tại đây để chờ xử lý." /></td></tr>
                    ) : (
                        ...
                        <tr key={b.id} className="hover:bg-gray-50/70 transition-colors">
                          ...
                          <td className="px-6 py-4">
                             <div className="text-[10px] text-gray-600">
                              ...
                              {b.items.filter(it => it.type === 'tool').length > 0 && (
                                <div>
                                  <div className="text-gray-500 font-bold uppercase mb-1">Lẻ:</div>
                                  {b.items.filter(it => it.type === 'tool').map((it, i) => (
                                    <div key={i} className="flex justify-between font-medium text-gray-700 bg-gray-50 px-1 rounded">• {it.name} <span>x{it.quantity}</span></div>
                                  ))}
                                </div>
                              )}
                            </div>
                          ...
                          <td className="px-6 py-4">
                             <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${
                               b.status === 'Requested' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'
                             }`}>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex space-x-2">
                               {b.status === 'Requested' ? (
                                  <button onClick={() => handleApproveBorrow(b.id)} className="px-3 py-1 bg-blue-600 text-white text-[10px] font-bold rounded hover:bg-blue-700 transition-colors">Duyệt mượn</button>
                               ) : (
                                  <button onClick={() => handleApproveReturn(b.id)} className="px-3 py-1 bg-blue-600 text-white text-[10px] font-bold rounded hover:bg-blue-700 transition-colors">Xác nhận trả</button>
                               )}
                               <button onClick={() => handleRejectRequest(b.id)} className="px-3 py-1 bg-red-50 text-red-600 text-[10px] font-bold rounded hover:bg-red-100 transition-colors">Từ chối</button>
                            </div>
                          </td>
                        </tr>
                      ))
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
               <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {liquidationLogs.length === 0 ? (
                    <tr><td colSpan={5}><EmptyState title="Chưa có lịch sử thanh lý" description="Các dụng cụ/gói được thanh lý hoặc báo hỏng sẽ ghi nhận tại đây." /></td></tr>
      ...
      case 'users':
        return <UserManagement users={users} departments={departments} currentUser={currentUser!} onRefresh={refreshData} />;
      case 'history':
        return (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
               <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100 text-xs">
                  {logs.length === 0 ? (
                    <tr><td colSpan={4}><EmptyState title="Chưa có nhật ký nào" description="Mọi thao tác thêm/sửa/duyệt trong hệ thống sẽ được ghi lại tại đây." /></td></tr>
                  ) : logs.map(log => (
                    <tr key={log.id} className="hover:bg-gray-50/70 transition-colors">
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 rounded-2xl border border-blue-100 shadow-xl shadow-blue-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100 hover:shadow-lg transition-all cursor-pointer group" onClick={() => exportToExcel(kits, 'Bao_cao_danh_muc_goi')}>
                  <div className="w-12 h-12 bg-gray-600 text-white rounded-xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform">
                  ...
                  <h4 className="font-black text-gray-900 mb-1">Danh mục gói dụng cụ</h4>
                  <p className="text-xs text-gray-500">Chi tiết các gói dụng cụ và thành phần bên trong.</p>
                </div>

                <div className="p-6 bg-blue-50 rounded-2xl border border-blue-100/50 hover:shadow-lg transition-all cursor-pointer group" onClick={() => exportToExcel(borrows, 'Bao_cao_lich_su_muon_tra')}>
                  <div className="w-12 h-12 bg-blue-500 text-white rounded-xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  </div>
                  <h4 className="font-black text-blue-900 mb-1">Lịch sử mượn trả</h4>
                  <p className="text-xs text-blue-700/70">Toàn bộ nhật ký mượn, trả và trạng thái hiện tại.</p>
                </div>

                <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100 hover:shadow-lg transition-all cursor-pointer group" onClick={() => exportToExcel(sterilizationLogs, 'Bao_cao_tiet_trung')}>
                  <div className="w-12 h-12 bg-gray-500 text-white rounded-xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                  </div>
                  <h4 className="font-black text-gray-900 mb-1">Báo cáo tiệt trùng</h4>
                  <p className="text-xs text-gray-500">Theo dõi thời gian và trạng thái xử lý dụng cụ.</p>
                </div>
              </div>
            </div>
          </div>
  ...
  const handleUpsertTool = async (tool: Partial<MedicalTool> & { id?: string }) => {
    try {
      await api.upsertTool(tool);
      await refreshData();
  ...
  const handleUpsertKit = async (kit: { id?: string; code: string; name: string; type: string; totalQuantity: number; items: { toolId: string; quantity: number }[] }) => {
    try {
      await api.upsertKit(kit);
      await refreshData();
  ...
  const handleAdjustStock = async (amount: number) => {
    if (!adjustingTool) return;
    try {
      await api.adjustStock(adjustingTool.id, amount);
      await refreshData();
  ...
  return (
    <Layout activeTab={activeTab} onTabChange={setActiveTab} user={currentUser} allowedTabs={allowedTabs} onLogout={handleLogout} pendingApprovalsCount={pendingCount}>
      {renderContent()}
      ...
      {showToolModal && (
        <ToolFormModal
          ...
          onSave={handleUpsertTool}
        />
      )}
      {showKitModal && (
        <KitFormModal kit={editingKit} tools={tools} onClose={() => { setShowKitModal(false); setEditingKit(null); }} onSave={handleUpsertKit} />
      )}
      {showAdjustModal && adjustingTool && (
        <QuantityAdjustmentModal tool={adjustingTool} mode={adjustMode} onClose={() => setShowAdjustModal(false)} onConfirm={handleAdjustStock} />
      )}
