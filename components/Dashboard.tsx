  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard label="Gói đang hoạt động" value={kitStats.activeKits} color="bg-green-600" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
        <StatCard label="Gói đang mượn" value={kitStats.borrowedKits} color="bg-purple-600" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
        <StatCard label="Gói chờ tiệt trùng" value={kitStats.waitingSterilization} color="bg-orange-600" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.183.15l-3.083 1.233A2 2 0 00.5 18.415V21a2 2 0 002 2h19a2 2 0 002-2v-2.585a2 2 0 00-1.072-1.788l-3.5-1.4z" /></svg>} />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-4 rounded-xl border border-gray-50 flex flex-col items-center justify-center text-center">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-2xl font-black text-green-600">{stats.returnedKits}</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-2xl font-black text-orange-600">{stats.waitingSterilizationSlips}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={kitData}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="Trong kho" fill="#10b981" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Đang mượn" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Chờ tiệt trùng" fill="#f59e0b" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="mt-4 space-y-2">
            {[
              { name: 'Khả dụng', value: kitStats.activeKits, color: '#10b981' },
              { name: 'Đang mượn', value: kitStats.borrowedKits, color: '#8b5cf6' },
              { name: 'Chờ tiệt trùng', value: kitStats.waitingSterilization, color: '#f59e0b' },
              { name: 'Hư hỏng', value: kitStats.damagedKits, color: '#ef4444' },
              { name: 'Đã thanh lý', value: kitStats.liquidatedKits, color: '#6b7280' },
